...
router.get('/getAll', async (req, res) => {
    try{
        ...
        return BadRequest(res, "Error getting all countries.", "REQUEST_ERROR", 400)
    }
...
router.get('/getOne/:countryCode', async (req, res) => {
    ...
    if (countryCode.length != 2) return BadRequest(res, "Invalid parameter: Country Code. Please provide a valid value.", "INVALID_DATA", 400)
    
    try{
        const borders = await axios.get(`https://date.nager.at/api/v3/CountryInfo/${countryCode}`)
        const populations = await axios.post('https://countriesnow.space/api/v0.1/countries/population', {
            country:borders.data.commonName    
        })
        const flag = await axios.post('https://countriesnow.space/api/v0.1/countries/flag/images', {
            iso2:countryCode
        })
        
        res.status(200).send({"borders": borders.data, "populations": populations.data, "flag": flag.data.data.flag})
    }catch(err){
